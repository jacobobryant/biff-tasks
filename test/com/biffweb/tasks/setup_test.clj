...
(deftest setup-renames-template-namespace-and-creates-missing-files
  (with-temp-dir [dir]
    ...
    (let [tailwind-called? (atom false)]
      ...
      (is (str/includes? (slurp-file dir ".git/hooks/pre-commit")
                         "clj -M:run format"))
      (is (str/includes? (slurp-file dir ".github/workflows/tests.yml")
...
(deftest setup-skips-namespace-rewrite-on-subsequent-runs
  (with-temp-dir [dir]
    ...
    (write-file dir ".github/workflows/tests.yml" "old workflow\n")
    (init-git! dir)
    (let [tailwind-called? (atom false)]
      (with-user-dir dir
        (with-redefs [util/read-config                           (constantly {:biff.tasks/main-ns 'com.acme-app})
      ...
      (is (str/includes? (slurp-file dir ".git/hooks/pre-commit")
                         "clj -M:run format"))
      (is (str/includes? (slurp-file dir ".github/workflows/tests.yml")
                         "autogenerated by biff-tasks setup")))))
